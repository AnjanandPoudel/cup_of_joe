  ...
  try {
    ...
    if ( token) {
      try {
        ...
        let admin=await Owner.findOne({_id:decoding._id})
        if(admin){
            req.admin=admin
        }
        let user=await User.findOne({_id:decoding._id})
        if(user){
            req.user=user
        }
        if(!admin && !user){
        ...
        throw new SetErrorResponse(`Access Not Granted ! Please Login Again: ${err}`,401);
      }
    }
    next();
  } catch (err) {
